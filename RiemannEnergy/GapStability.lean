...
theorem Gap_stability
  ...
  have hε_le_M : ε ≤ hData.Mδ := by
    simpa [ε, εδ] using (min_le_left hData.Mδ (hData.cδ / (8 * hData.Mδ)))

  have hε_le_frac : ε ≤ hData.cδ / (8 * hData.Mδ) := by
    simpa [ε, εδ] using (min_le_right hData.Mδ (hData.cδ / (8 * hData.Mδ)))
  ...
  have hVWfun : (fun x => V x + (W x - V x)) = W := by
    funext x
    -- V + (W - V) = V + W - V = W
    simpa [add_sub_assoc] using (add_sub_cancel (V x) (W x))
  ...
  have hPert3 :
    ...
    have hsimp :
      ...
      field_simp [hMne]
      ring
    exact le_trans hmul (by simpa [hsimp])
