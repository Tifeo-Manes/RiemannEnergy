...
theorem cs_integral_mul_real {f g : ℝ → ℝ}
  ...
  have _hA : 0 ≤ A := integral_nonneg (fun x => by positivity)
  ...
  have hC2 : C ^ 2 ≤ A * B := by
    ...
    · have hBpos : 0 < B := lt_of_le_of_ne hB (Ne.symm hB0)
      ...
      have hdiv : C ^ 2 / B ≤ A := by linarith [hQ'']
...
theorem abs_integral_mul_le {f g : ℝ → ℝ}
  ...
  have _hB : 0 ≤ B := integral_nonneg (fun x => by positivity)
...
theorem sqrt_integral_minkowski {f g : ℝ → ℝ}
  ...
  calc
    ...
    _ = Real.sqrt A + Real.sqrt B := by
          have hnonneg : 0 ≤ Real.sqrt A + Real.sqrt B := by positivity
          simp [hnonneg]
